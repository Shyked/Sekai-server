...
(function(window){
    ...
    function now(){
        ...
        return (perf && perf.now) ?
            (perf.now() + (perf.timeOrigin || perf.timing.navigationStart)) :
            Date.now();
    }
        ...
        collideBodies: function(bodyA, bodyB, normal, point, mtrans, contact){
            ...
            if ( cof && vreg ){
                ...
                if ( fixedA ){
                    ...
                    bodyB.state.angular.vel *= 0.9999;
                    if (Math.abs(bodyB.state.angular.vel) > 0.00001) bodyB.state.angular.vel -= Math.sign(bodyB.state.angular.vel) * 0.00001;
                    ...
                    bodyA.state.angular.vel *= 0.9999;
                    if (Math.abs(bodyA.state.angular.vel) > 0.00001) bodyA.state.angular.vel -= Math.sign(bodyA.state.angular.vel) * 0.00001;
