(function() {
			...
			if (!worldJSON.entitiesCount) {
				...
				for (var idE in worldJSON.entities) {
					if (worldJSON.entities[idE].LOAD) {
						loadedEntity = singleLoader.loadEntitySync(worldJSON.entities[idE].LOAD, worldJSON.id);
	...
	Loader.prototype.loadEntitySync = function(entityName, worldId) {
		if (worldId) {
			...
			else return clone(this.loadedEntities[worldId + "/" + entityName]);
		}
		try {
		...
		return clone(this.loadedEntities[entityName]);
	};
