		...
		for (var idE in this.entities) {
			...
			if (!this.entities[idE].rotation) {
				if (this.type == "circular") {
					...
					//toTarget = Math.min(toTarget, Math.PI * 2 - toTarget); // Shortest distance to 0PI
					//toTarget = (toTarget > Math.PI) ? - Math.PI * 2 - toTarget : toTarget;
					this.entities[idE].physicsBody.state.angular.pos = this.entities[idE].physicsBody.state.angular.pos + toTarget;
					this.entities[idE].physicsBody.state.old.angular.pos = this.entities[idE].physicsBody.state.old.angular.pos + toTarget;
				}
				else if (this.type == "flat") {
